...
var server = http.createServer(function (request, response) {
  ...
  console.log("有个xx发请求过来啦！路径（带查询参数）为：" + pathWithQuery);

  /*所有path的开头的'/'均不能省略，不然会访问不到 */
  if (path === "/index.html") {
    ...
    string = string.replace("{{page1}}", `<ul id="ulhere">${result}</ul>`);
    response.write(string);
    response.end();
  } else if (path === "/main.js") {
    response.statusCode = 200;
    response.setHeader("Content-type", "text/javascript;charset=utf-8");
    let str = fs.readFileSync("public/main.js");
    response.write(str);
    response.end();
